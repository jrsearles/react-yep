...
- Not bound to form rendering - can be wrapped around any set of components you'd like to validate against a specified schema. (For example, this can easily be used for validating rows in an editable table or list.)
- Built with localization in mind.

> This library is intended to be used with validation that is invoked on submit.
...
import { ValidationProvider } from "react-yep";
...
The `ValidationProvider` is the primary component which wraps the children intended to be validated. The component will provide render props to a function (see [Render Props](#render-props)). The component can also be used without render props, with the `useValidation` hook available to access the same validation context provided through the render props.
...
| t      |  function  |    no    | Message formatter for localization purposes. `(key: string, params: object) => string` (see: [Localization](#localization) for more information) |
...
| ------------- | :--------------------------------------------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| validate      | `(data: any, context?: object) => Promise{boolean}`  | Calls the [validate](https://github.com/jquense/yup#mixedvalidatevalue-any-options-object-promiseany-validationerror) method on the `schema` object. If provided, the `context` parameter will be passed along to the `validate` method. Returns `true` if the validation passed, otherwise `false`. Calling this method will update the component's state, setting error messages which can be read by child components. |
...
| hasErrors     |             `(key?: string) => boolean`              | Indicates whether the specific key has errors. If a key is nor provided, it will indicate whether the data has any errors.                                                                                                                                                                                                                                                                                                |
| getErrors     |          `(key: string) => ErrorMessage[]`           | Gets the error messages for the provided key. If not errors exist, an empty array will be returned.                                                                                                                                                                                                                                                                                                                       |
...
| isValidating  |                   `() => boolean`                    | Indicates whether validation is being performed. Since validation can be asyncronous, this allows you to disable form elements while validation is being performed.                                                                                                                                                                                                                                                       |
...
| label              |           `string`           |    no    | The label to use for the control for validation messages. (If not provided, the text from an associated `HTMLLabelElement` will be used if found.) |
| className          |           `string`           |    no    | The class to apply to the component.                                                                                                               |
| style              |       `CSSProperties`        |    no    | The style to apply to the component. (default style is set to `display: inline-block`)                                                             |
| errorClassName     |           `string`           |    no    | Class name which is applied to the wrapping div when in error.                                                                                     |
| errorListClassName |           `string`           |    no    | Class name which is applied to the error list for the control.                                                                                     |
| requiredClassName  |           `string`           |    no    | Class name which is applied to wrapping div when control is required.                                                                              |
| requiredClassName  | `{ [name: string]: string }` |    no    | Additional classes which are applied to the control based off of the validation applied to the control -> class name.                              |
...
This component is rendered as an unordered list (`UL`) with a list item for every error found for the provided name. (This is used by the `Control` component.)
